...
class TestTimeSeries(WalrusTestCase):
    ...
    def test_timeseries_ranges(self):
        ...
        self.assertRecords(self.ts.sa.range(), [6, 9])
        self.assertRecords(self.ts.sa.range(count=1), [6])
    ...
    def test_timeseries_read(self):
        ...
        self.ts.reset()
        self.assertRecords(self.ts.read(), list(range(1, 10)))
        ...
        self.ts.reset(datetime.datetime(2018, 1, 4))
        self.assertRecords(self.ts.read(), [4, 6, 8, 9])
    ...
    def test_adding(self):
        ...
        r = self.ts.sb.add({'k': 'sa-11'}, (datetime.datetime(2018, 1, 10), 2))
        ...
        self.assertEqual(r11.data, {'k': 'sa-11'})
    ...
    def test_timeseries_stream_read(self):
        ...
        # Read two from sa, one from sc, then read 2x from all. Records that
        # were read will not be re-read.
        self.assertRecords(self.ts.sa.read(count=2), [0, 3])
        self.assertRecords(self.ts.sc.read(count=1), [2])
        self.assertRecords(self.ts.read(count=2), [1, 4, 5, 6, 8, 9])
        self.assertRecords(self.ts.read(count=1), [7])
        ...
        self.assertRecords(self.ts.read(), [4, 7])

        # Re-set the ID of stream a and trim.
        self.ts.sa.set_id('0')
        self.ts.sa.trim(2, False)
        self.assertRecords(self.ts.read(), [6, 9])
    ...
    def test_ack_claim_pending(self):
        ...
        self.assertRecords(ts1.sa.read(1), [0])
        self.assertRecords(ts2.sb.read(2), [1, 4])
        self.assertRecords(ts2.sa.read(1), [3])
        ...
        self.assertRecords(ts2.read(), [2, 5, 6, 7, 8, 9])
        assertPending(ts2.sa.pending(), [
