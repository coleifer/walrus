...
class TestSearchIndex(WalrusTestCase):
    def test_search_index(self):
        ...
        def assertDocs(query, indexes):
            results = [decode(key['content']) for key in index.search(query)]
    ...
    def test_add_remove_update(self):
        ...
        self.assertEqual(decode(huey['content']), 'huey cat')
        self.assertEqual(decode(huey['type']), 'cat')
        self.assertEqual(decode(huey['color']), 'white')

        cats = idx.search('cat')
        self.assertEqual([decode(doc['content']) for doc in cats],
                         ['zaizee cat cat', 'huey cat', 'beanie cat'])

        idx.remove('3')  # Poor beanie :(
        zaizee, huey = idx.search('cat')
        self.assertEqual(decode(zaizee['content']), 'zaizee cat cat')
        self.assertEqual(decode(huey['content']), 'huey cat')
        ...
        zaizee, huey = idx.search('cat')
        self.assertEqual(decode(zaizee['content']), 'zaizee cat')
        self.assertEqual(decode(zaizee['type']), 'kitten')
        self.assertEqual(decode(zaizee['color']), 'gray')

        self.assertEqual(decode(huey['content']), 'huey baby cat')
    ...
    def test_search_parser(self):
        ...
        def assertMatches(query, expected):
            results = [decode(doc['content']) for doc in index.search(query)]
        ...
        results = [decode(doc['content']) for doc in index.search('')]
